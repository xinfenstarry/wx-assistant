...
class UserPreferences(context: Context) {
    ...
    var lastChatGptHandoffAt: Long
        get() = preferences.getLong(KEY_LAST_HANDOFF_AT, 0L)
        set(value) = preferences.edit().putLong(KEY_LAST_HANDOFF_AT, value).apply()

    fun beginChatGptExchange(now: Long = System.currentTimeMillis()): String {
        val token = UUID.randomUUID().toString()
        preferences.edit()
            .putString(KEY_EXCHANGE_TOKEN, token)
            .putLong(KEY_EXCHANGE_CREATED_AT, now)
            .apply()
        return token
    }

    fun isValidChatGptExchange(token: String, now: Long = System.currentTimeMillis()): Boolean {
        val pending = preferences.getString(KEY_EXCHANGE_TOKEN, null) ?: return false
        val createdAt = preferences.getLong(KEY_EXCHANGE_CREATED_AT, 0L)
        return token == pending && createdAt > 0L && now - createdAt in 0L..EXCHANGE_TTL_MS
    }

    fun consumeChatGptExchange(token: String) {
        if (preferences.getString(KEY_EXCHANGE_TOKEN, null) != token) return
        preferences.edit()
            .remove(KEY_EXCHANGE_TOKEN)
            .remove(KEY_EXCHANGE_CREATED_AT)
            .apply()
    }
    ...
    companion object {
        ...
        private const val KEY_LAST_HANDOFF_AT = "last_chatgpt_handoff_at"
        private const val KEY_EXCHANGE_TOKEN = "chatgpt_exchange_token"
        private const val KEY_EXCHANGE_CREATED_AT = "chatgpt_exchange_created_at"
